...
extension WebView.Coordinator: WKScriptMessageHandler{
    func userContentController(_ userContentController: WKUserContentController,
        ...
        //리액트의 index.html에 토큰 전달해줌
        if message.name == "setLoginInfo" {
            // 대리자의 필수 정의인 received
            delegate?.receivedJsonValueFromWebView2(value: message.body as! [String : Any?])
            print("message 내용: \(message.body)")

//         String(describing: value["zonecode"]!!)
            print("2222")
        }
...
//    @StateObject var responseModel: ReponseModel = ReponseModel()
    ...
    //리액트 index.html에 토큰 전달
    func receivedJsonValueFromWebView2(value: [String: Any?]) {
        self.viewModel.foo.send("login")   //toNative() 호출인데 안에 ("login") 있어야함
    ...
    //여기에 switch로 메시지 별로 불러오게..? 먼저 foo bar 해서 toNative
    //우편번호
    func receivedJsonValueFromWebView(value: [String : Any?]) {
        
        print("from javascript JSON : \n", value)
        
        //index.html에  messageHandlers에 "example"로 해놓았으므로 작동하긴 함
        //우편번호 + 도로명주소
        print(type(of: value))  //dictionary
        print(value["zonecode"]!!)   //우편번호
        print(value["roadAddress"]!!)  //도로명 주소
        
        //우편번호
        //let
        let zoneCode = String(describing: value["zonecode"]!!)
        postCodeModel.zipcode = zoneCode
        print("postCodeModel.zipcode: \(postCodeModel.zipcode)")
        
        
        //주소
        let  roadAddress = String(describing: value["roadAddress"]!!)
        postCodeModel.adderss = roadAddress
        ...
        if let message = value["message"] as? String{
            ...
            self.viewModel.bar.send(String(message))
        }
    }
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        configuration.userContentController.add(self.makeCoordinator(), name: "setLoginInfo")
    ...
    class Coordinator : NSObject, WKNavigationDelegate {
        ...
        func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
            ...
            print("a:\(parent.token)")
            print("b:\(parent.membSn)")
            
            let a = ChargeModel(accessToken: parent.token, membSn: parent.membSn, membCls: parent.membCls)
            
            //model로 받아서 보내느 거 테스트
            
            
            let jsonData: Data = try! JSONEncoder().encode(a)

            let jsonString:String = String.init(data:jsonData, encoding: .utf8) ?? "err"
            
            
           // 위에서 foo ("login")한 걸 받아와서 receive로 다시 react에 token전달해주는 걸로 일단 이해함.
            self.foo =  self.parent.viewModel.foo.receive(on: RunLoop.main).sink(receiveValue: { value in
                ...
                print("datadatadata:\(jsonString)")
            })
            
            
            // 웨뷰에 토큰이 한 번에 보내지지 않으므로 , 위의 self.foo 를  리액트에서 먼저 보내고, 다시 receive로 swift에서 리액트로 토큰, membSn을 보낸다.
            
            //            webView.evaluateJavaScript("fromNative('\(data)')", completionHandler: { result, error in
            //                if let anError = error {
            //                    print("Error \(anError.localizedDescription)")
            //                }
            //                print("data:\(data)")
